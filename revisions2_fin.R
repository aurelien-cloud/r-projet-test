...
don <- read_csv2("C:/Users/cepe-s4-09/Desktop/ravary_12052025/bank.csv")
...
ma_foret <- randomForest(data = don, Y ~ ., type="classification")
...
bank2 <- select(don, age, balance, duration, y) |>
  mutate(Y = if_else(y == "no", 0, 1)) %>%
  select(-y)

# library(ranger)

modele_choisi <- ranger(Y ~ ., 
                        data = bank2,
                        num.trees = 100,
                        mtry = 2,
                        min.node.size = 13,
                        sample.fraction = 0.8981604,
                        importance = "permutation",
                        num.threads = 8,
                        oob.error = TRUE)

imp <- importance(modele_choisi)

par(cex = 0.75, mar = c(9, 4, 4, 2) + 0.1)

options(scipen = -2) 

# 3. Barplot avec textes et titre plus petits, espace réduit
barplot(sort(imp, decreasing = TRUE),
        las       = 2,
        space     = 0.4,
        cex.names = 0.9,
        cex.axis  = 0.9,
        cex.main  = 1.5,
        main      = "Importance des variables (permutation)")

plot(modele_choisi$predictions)

# avoir les codes nickel qui tournent
# je mets tous les modèles
# paramètres optimaux
# xgboost : avec profondeur de 1, step de 0.1 et j'envoie l'itération
# on trouve le meilleur modèle, et je prévois le truc test
# on fait du feature (carré, cube, spline qui permettent d'avoir de la non-linéarité)
# s'il y a des groupes, choix de modèle par groupe

# PROJET :
# données, problèmes, comment j'ai tenté de le résoudre
